...
def analysis(strike,option_type):
    global df_opt,lot_size,df_trade
    ...
    while exit != True:
        ...
        if ltp_change < 0:
            ...
            sl_pricemv = min(sl_price0 + step * (1 + math.floor(math.exp(1000*(pp_change)))),ltp - 2*step)
...
file_name = f"Test_Comm_Output_{datetime.datetime.strftime(today,'%Y_%m_%d')}.csv"
