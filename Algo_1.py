...
test = pd.DataFrame({'order_id':[order_id],'BUY_PRICE':[buy_price],'QTY':[qty],'LTP':[ltp],'PP':[pp],'SLP': [slp],"TPP":[tpp],"SL_PRICE":[sl_price],'TRIGGER_PRICE':[trigger],'TP_PRICE':[tp_price],"SELL_PRICE":[np.nan],"P/L":[np.nan]})
test
...
while exit == False:
    ...
    if ltp <= trigger:
        sell_price = ltp
        exit = True
        test.loc[len(test.index),:] = [order_id,buy_price,qty,ltp,pp,slp,tpp,sl_price,trigger,tp_price,sl_price,(sl_price - buy_price)*qty]
        break
    if pp >= tpp:
        tpp = pp + 0.05
    ...
    test.loc[len(test.index),:] = [order_id,buy_price,qty,ltp,pp,slp,tpp,sl_price,trigger,tp_price,np.nan,np.nan]
